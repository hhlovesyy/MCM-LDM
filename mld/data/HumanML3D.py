...
import os # For os.path.isabs
from mld.data.humanml.scripts.motion_process import (process_file,
                                                     recover_from_ric,
                                                     extract_features)
from typing import Optional, Union, Any, Callable, List, Tuple, Dict # 根据你实际使用的类型添加
from .base import BASEDataModule
from .humanml.data.dataset import Text2MotionDatasetV2, TextOnlyDataset, HumanML3DSceneDataset

class HumanML3DSceneDataModule(BASEDataModule):
    """
    LightningDataModule for the HumanML3D-Scene dataset.
    ...
    def __init__(self,
                 ...
                 data_root: str,  # 'datasets/humanml3d_scene'
                 motion_dir_name: str, # 'new_joint_vecs'
                 text_dir_name: str, # 'texts'
                 scene_label_filename: str, # 'scene_labels.json'
                 num_scene_classes : int, # 100
        ...
        # Set the dataset name, used by BASEDataModule's __getattr__ to construct
        # paths from cfg.
        # e.g., cfg.DATASET.{self.name.upper()}.SPLIT_ROOT
        # For our scene dataset, we will manage paths more directly using
        # self.hparams.data_root but self.name is still good to have for identification.
        self.name = "humanml3d_scene"
        
        # Store the main config object if needed for other logic
        self.cfg = cfg
        
        # Set the Dataset class to be used by BASEDataModule's __getattr__
        self.Dataset = HumanML3DSceneDataset
        
        # Set number of joints (typically fixed for HumanML3D/SMPL)
        self.njoints = 22  # Standard for SMPL model used in HumanML3D
        
        # Determine nfeats by creating a sample dataset instance.
        # This mimics the behavior of HumanML3DDataModule.
        # We use self.get_sample_set() for this, which needs to be adapted.
        sample_overrides_for_nfeats = {
            # Use the validation split filename for creating the sample set
            "split_filename_override": self.hparams.split_val_filename,
    ...
    def get_sample_set(self, overrides: dict = None) -> HumanML3DSceneDataset:
        """
        ...
        # Construct arguments for the HumanML3DSceneDataset constructor using dictionary access
        dataset_args_for_sample = {
            "mean": sample_params_dict['mean'], # <--- 使用字典访问
            "std": sample_params_dict['std'],   # <--- 使用字典访问
            "w_vectorizer": sample_params_dict['w_vectorizer'], # <--- 使用字典访问
            "split_file": actual_split_file_path,
            "max_motion_length": sample_params_dict['max_motion_length'], # <--- 使用字典访问
            "min_motion_length": sample_params_dict['min_motion_length'], # <--- 使用字典访问
            "max_text_len": sample_params_dict['max_text_len'],           # <--- 使用字典访问
            "unit_length": sample_params_dict['unit_length'],             # <--- 使用字典访问
            ...
            "scene_label_filepath": pjoin(sample_params_dict['data_root'], sample_params_dict['scene_label_filename']), # <--- 使用字典访问
            "num_scene_classes": sample_params_dict['num_scene_classes'],   # <--- 使用字典访问
    ...
    def __getattr__(self, item: str):
        ...
        kwargs for HumanML3DSceneDataset.
        """
        if item.endswith("_dataset") and not item.startswith("_"):
            ...
            if cached_item_name not in self.__dict__:
                ...
                dataset_constructor_args = {
                    ...
                    "scene_label_filepath": pjoin(self.hparams.data_root, self.hparams.scene_label_filename),
                    "num_scene_classes": self.hparams.num_scene_classes,
