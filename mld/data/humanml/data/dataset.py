...
class Style100Dataset(data.Dataset):
    def __init__(
        ...
        style_label_filepath, # Path to your scene label mapping file (e.g., "scene_mappings.txt")
        style_classes=100, # Number of unique scene categories
        ...
        **kwargs, # To catch any other arguments like dataset_name if used by w_vectorizer etc.
        ...
        self.max_text_len = max_text_len
        self.unit_length = unit_length
        self.num_scene_classes = style_classes
        ...
        with open(style_label_filepath, 'r') as f:
            for line in f:
                ...
                self.style_label_map[actual_motion_id] = scene_id
        ...
        for i, name in enumerate(id_list): # 'name' here is the motion_id like "000001"
            ...
            try:
                ...
                if name not in self.style_label_map:
                ...
                self.data_dict[name] = {
                    ...
                    "style_label": self.style_label_map[name] # Store the scene label
                }
                new_name_list.append(name)
                length_list.append(len(motion))
    ...
    def __getitem__(self, item):
        ...
        text_list = data["text"]
        style_label = data["style_label"] # Get the scene label
        ...
        # 在单独训练DCE模块（包含预训练CStyle分类器的时候），并不需要归一化操作（在训练脚本中有进行归一化）
        motion_normalized = motion_segment
